...
var url = "mongodb://<username>:<password>@ds021691.mlab.com:21691/barno-battles"; // TODO: change to env var
mongo.connect(url, function(err, db) {
    if (!err) {
        console.log("connected to db");
        var collections = {
            Lobby: db.collection("lobby-history"),
            Suggestions: db.collection("suggestions-history"),
            users: db.collection("users")
        };
        startSocket(collections);
    } else {
        console.log(err);
    }
});
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('chatroom message', function(name, msg, room) { // TODO: sanitize for HTML input, add a roomname param and only send to that room
            ...
            if (msg.startsWith('/')) { // include ! command to broadcast?
                ...
                collections[room].insert({time: dt, user: name, msg: msg});
                msg = "<b>" + name + ":</b> " + msg;
